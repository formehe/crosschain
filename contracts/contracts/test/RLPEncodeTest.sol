...
contract RLPEncodeTest {
 ...
 /*
     ...
     */
    function encodeLength(uint len, uint offset) public pure returns (bytes memory) {
        bytes memory encoded;
        if (len < 56) {
            encoded = new bytes(1);
     ...
     * @return RLP encoded bytes.
     */
    function toBinary(uint _x) public pure returns (bytes memory) {
     ...
     */
    function flatten(bytes[] memory _list) public pure returns (bytes memory) {
        if (_list.length == 0) {
            return new bytes(0);
        }
